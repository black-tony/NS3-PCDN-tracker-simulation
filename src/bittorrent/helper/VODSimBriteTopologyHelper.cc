...
TypeId VODSimBriteTopologyHelper::GetTypeId (void)
{
...
NodeContainer VODSimBriteTopologyHelper::AddClientNodesToLastTopology (uint32_t numberOfClientNodes, std::string deviceType)
{
...
NodeContainer VODSimBriteTopologyHelper::AddNodeToLastTopology (Ptr<Node> parent, std::string deviceType, std::string upstreamBandwidth, std::string downstreamBandwidth, std::string delay, bool isRouterNode, bool isBorderRouterNode, int32_t AS, bool isTrackerNode, std::string specialIP)
{
...
NodeContainer VODSimBriteTopologyHelper::AddTapNodeToLastTopology (std::string tapDeviceName, Ptr<Node> parent, std::string upstreamBandwidth, std::string downstreamBandwidth, std::string delay, bool isRouterNode, bool isBorderRouterNode, int32_t AS)
{
  Ptr<Node> newNode = CreateObject<Node> ();
  NodeContainer nodeContainer;
  nodeContainer.Add (newNode);
...
bool VODSimBriteTopologyHelper::HasTapNode () const
{
  return m_hasTapNode;
}

bool VODSimBriteTopologyHelper::EstablishNetworkOnLastTopology ()
{
...
std::list<std::pair<std::string, std::string> > VODSimBriteTopologyHelper::GetBandwidthDistributionFromFile (uint32_t numberOfNodes)
{
...
std::list<std::pair<std::string, std::string> > VODSimBriteTopologyHelper::GetSimpleDSLDistribution (uint32_t numberOfNodes)
{
  std::list<std::pair<std::string, std::string> > result;
...
void VODSimBriteTopologyHelper::SetBandwidthSamplesFile (std::string bandwidthSamplesFile)
{
  m_bandwidthSamplesFile = bandwidthSamplesFile;
}

std::string VODSimBriteTopologyHelper::GetBandwidthSamplesFile () const
{
  return m_bandwidthSamplesFile;
}

void VODSimBriteTopologyHelper::SetClientLinkDelays (uint32_t minDelay, uint32_t maxDelay)
{
...
uint32_t VODSimBriteTopologyHelper::GetClientLinkDelaysMin () const
{
  return m_minDelay;
}

uint32_t VODSimBriteTopologyHelper::GetClientLinkDelaysMax () const
{
  return m_maxDelay;
}

void VODSimBriteTopologyHelper::EnablePcaps (std::string pcapFilePrefix)
{
  m_pcapFilePrefix = pcapFilePrefix;
}

std::string VODSimBriteTopologyHelper::GetPcapFilePrefix () const
{
  return m_pcapFilePrefix;
}

void VODSimBriteTopologyHelper::DistributeRouterNodesToMPISystems()
{
#ifdef NS3_MPI
  ...
  for(std::map<std::string, Ptr<Node> >::iterator it = m_routers.begin(); it != m_routers.end(); ++it)
  {
    ...
    NS_LOG_INFO("VODSimBriteTopologyHelper: AS " << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] << " routers execute on CPU " << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize() << ".");
  }
  for(std::map<std::string, Ptr<Node> >::iterator it = m_borderRouters.begin(); it != m_borderRouters.end(); ++it)
  {
      ((*it).second)->SetAttribute ("SystemId", UintegerValue(m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize()));
      NS_LOG_INFO("VODSimBriteTopologyHelper: AS " << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] << " border routers execute on CPU " << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize() << ".");
  }
#endif
}

void VODSimBriteTopologyHelper::DistributeClientNodesToMPISystems()
{
#ifdef NS3_MPI
  if(MpiInterface::GetSize() == 1)
  {
  ...
  for(std::map<std::string, Ptr<Node> >::iterator it = m_clients.begin(); it != m_clients.end(); ++it)
  {
    ...
    NS_LOG_INFO("VODSimBriteTopologyHelper: Node " << ((*it).second)->GetId() << " executes on CPU " << m_clientsToRouters[lexical_cast<std::string>(((*it).second)->GetId())]->GetSystemId() << ".");
  }
#endif
}

bool VODSimBriteTopologyHelper::WriteLastTopologyToGraphVizFile (std::string fileName) const
{
